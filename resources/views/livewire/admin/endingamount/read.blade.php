    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">اعمال الانهائات</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded" style="background-color: #e9ecef!important;">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">اعمال الانهائات</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('endingamount')->create)
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.endingamount.create')?project_id={{$project_id}}&category_id={{$category_id}}&worker_id={{$worker_id}}" class="btn btn-success">{{ __('CreateTitle', ['name' => __('EndingAmount') ]) }}</a>
                        </div>
                        @endif
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    ...
                    @foreach($endingamounts as $endingamount)
                        @livewire('admin.endingamount.single', ['endingamount' => $endingamount], key($endingamount->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
            <div class="m-auto pt-3 pr-3">
                {{ $endingamounts->appends(request()->query())->links() }}
            </div>
