    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">واردات المكتب</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded" style="background-color: #e9ecef!important;">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">واردات المكتب</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('projectperctanges')->create)
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.projectperctanges.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Projectperctanges') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('projectperctanges')->searchable())
                        <div class="col-md-4">
                        ...
                        <div class="col-md-6">
                            <div class='form-group'>
                                <label for='perctange' class=' control-label'>المجموع</label>
                                <input readonly  class='form-control' value="@convert($total)">
                            </div>
                        </div>
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    <tbody>
                    <tr>
                        <td style='cursor: pointer' wire:click="sort('project_id')"> <i class='fa @if($sortType == 'desc' and $sortColumn == 'project_id') fa-sort-amount-down ml-2 @elseif($sortType == 'asc' and $sortColumn == 'project_id') fa-sort-amount-up ml-2 @endif'></i> {{ __('Project_id') }} </td>
                    ...
                    @foreach($projectperctangess as $projectperctanges)
                        @livewire('admin.projectperctanges.single', ['projectperctanges' => $projectperctanges], key($projectperctanges->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
            <div class="m-auto pt-3 pr-3">
                {{ $projectperctangess->appends(request()->query())->links() }}
            </div>
