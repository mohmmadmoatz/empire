<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('expensecategory')->delete or getCrudConfig('expensecategory')->update)
        <td>
            ...
            @if(getCrudConfig('expensecategory')->update)
                <a href="@route(getRouteName().'.expensecategory.update', ['expensecategory' => $expensecategory->id])" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('expensecategory')->delete)
